/*****************************************************************************************
	...
	Fully reversible copolymerization - 
		forward parameters kAA+AA (kAA)	kAA+BB (kAB)	kBB+BB (kBB)	kBB+AA (kBA)
		bakward parameters kA-AA (kA-A)	kA-BB (kA-B)	kB-BB (kB-B)	kB-AA (kB-A)
...
double		residual_time(VectorXd x_data,VectorXd y1_data,VectorXd y2_data,
					VectorXd x_model,VectorXd y1_model,VectorXd y2_model);
...
int main(int argc, char* argv[])
{
	...
	int			number = 100; // higher for lower error/noise
	...
	conv_model_all = conversion(concentrations,conv_model_A,conv_model_B);
	r =	residual_time( time_data, conv_data_A, conv_data_B, time_model, conv_model_A,conv_model_B);
	// output
	cout << "********************** RESULTS *********************" << endl;
...
MatrixXd	stochastic8(VectorXd concentrations,int i_number,VectorXd parameters,double t_final)
{
	...
	vector<polymer>		cycles(chains);  //cyclic polymer chains - contains cycles
	vector<int>			cyclesIndexArray(0); // contains index of non-zero cycles
	//cycles.size() is not reflective of actual number of cycles because you need it to have 1 row just for it to exist. So instead separately track number of cycles
	int					ncycles = 0;
	...
	while (t < termination_time)
	{
		...
		{
			nE += cycles[i].getDegreeOfPolymerization();
		...
		rho(7) = kAmB_d * nPABB * cat_ratio; // 8
		rho(8) = kBmA_d * nPBAA * cat_ratio; // 9
		rho(9) = kBmB_d * nPBBB * cat_ratio; // 9
		...
		else if (b > lower[10] && b <= upper[10])
		{
			...
			// initiator to chain transesterification
			if (polymers[selected_chain].hasInitiated() == false)
			{
				// chain initiates by transesterification
				// new chain
				I = I - 1;
				initiated.push_back(selected_chain);
				...
				//
				if (!ester_found) // then check the cyclics for it
				{
					for (int i=0;i<=cycles.size()-1;i++)
					{
						// NOTE cycles[] is a very sparse array. Should use a lookup table
						// of just the nonzero elements. Set up where the cycles[]
						// are first instantiated (isInitiated) and add to 
						// a cyclesIndexArray[] = {5, 543, 32, 5, ... }
						ester_sum += cycles[i].getDegreeOfPolymerization();
						if (ester_sum >= selected_ester)
						{
							chain2 = i;
							ester_sum -= cycles[i].getDegreeOfPolymerization();
							ester_id = selected_ester - ester_sum;
							//cout << "selected_ester = " << selected_ester <<  "\t" "ester_id ic = " << ester_id << " on chain " << chain2 << endl;
							ester_found = true;
							is_cyclic = true; // this will trigger
						}
					}
				}
				// for loops to steal sequence from ester_id to end and add to a 
				// find chain with ester, start there, transfer to selected_chain
				if (ester_found && !is_cyclic) // it's on a linear chain A
				{
				...
				else if (ester_found && is_cyclic) // adds the whole cycle and to selected_chain B
				{
					// start at ester_id on chain2 (cycle) until "end"
					for (int i=ester_id;i<=cycles[chain2].getDegreeOfPolymerization()-1;i++)
					{
						if (cycles[chain2].repeatUnitAtPosition(i)==Atype)
						{
							polymers[selected_chain].addMonomerToEnd(Atype);
							repeats_added += 1;
						}
						else if (cycles[chain2].repeatUnitAtPosition(i)==Btype)
						{
							polymers[selected_chain].addMonomerToEnd(Btype);
							repeats_added += 1;
						}
					}
					// get the rest of the cycle - this preserves the geometry and 
					// directionality of the transesterification linear/cyclic
					for (int i=0;i<=ester_id-1;i++)
					{
						if (cycles[chain2].repeatUnitAtPosition(i)==Atype)
						{
							polymers[selected_chain].addMonomerToEnd(Atype);
					...
					// trim chain2 from end to length transfered to selected_chain
					ester_sum = cycles[chain2].getDegreeOfPolymerization(); // reusing  variable
					for (int i=0;i<=ester_sum-1;i++) // count up - the whole thing
					{
						// NOTE possibility for non-conservation of repeat units here
						cycles[chain2].removeMonomerFromEnd();
						repeats_removed += 1;
					}
					// does not create free initiator
					if (repeats_added != repeats_removed)
					{
			...
			else // chain-to-chain transesterification
			{
				//		no. of esters is monomers that have reacted 
				//		nE = (nA0 + nB0) - (A + B)
				// find chain2 with selected_ester
				for (int i=0; i<=polymers.size()-1;i++)
				{
					ester_sum += polymers[i].getDegreeOfPolymerization();
					// we are searching for the polymer with 
				...
				{
				cout << "before" << endl;
				cout << "to: size of selected chain "<< selected_chain << " DP = " << polymers[selected_chain].getDegreeOfPolymerization() << endl;
				polymers[selected_chain].printPolymer(); // it seems sometimes this chain does not exist or has no repeat units
				cout << "from: size of reactant chain " << chain2 << " DP = " << polymers[chain2].getDegreeOfPolymerization() << endl;
				polymers[chain2].printPolymer();
				cout << "transfer of " << polymers[chain2].getDegreeOfPolymerization()-ester_id << endl;
				}
				// for loops to steal sequence from ester_id to end and add to a 
				// find chain with ester, start there, transfer to selected_chain
				if (selected_chain == chain2)
				{
					// intramolecular chain transesterification.
					// if chain1 == chain2 then form a cyclic-dormant and release linear-active
					// One chain becomes two, one active, one dormant (cycle)
					// NOTE If selected_chain == chain2 twice, then we need a different cycle. This adds to existing.
					// solution: If cycles[chain2].hasInitiated() then search forward for an empty spot. drop there.
					// transfer esters to cycles container
					if (!cycles[chain2].hasInitiated()) // C
					{
						for (int i=ester_id;i<=polymers[chain2].getDegreeOfPolymerization()-1;i++)
						{
							if (polymers[chain2].repeatUnitAtPosition(i)==Atype)
							{
								cycles[selected_chain].addMonomerToEnd(Atype);
								repeats_added += 1;
							}
							else if (polymers[chain2].repeatUnitAtPosition(i)==Btype)
							{
								cycles[selected_chain].addMonomerToEnd(Btype);
								repeats_added += 1;
							}
						}
					}
					else // search for empty cycle spot
					{
						// search for empty cycle spot randomly
						// chain3 should replace selected_chain in reference to cycles[]
						chain3 = int(double(chains) * double(rando()) / rando_maximo);
						while (cycles[chain3].hasInitiated()==true)
						{
						// NOTE working here
							chain3 = int(double(chains) * double(rando()) / rando_maximo);
							if (cycles[chain3].hasInitiated()==false)
							{
								break;
							}
						}
						// transfer repeat units from polymers[chain2] to cycles[chain3]
						for (int i=ester_id;i<=polymers[chain2].getDegreeOfPolymerization()-1;i++)
						{
							if (polymers[chain2].repeatUnitAtPosition(i)==Atype)
							{
								cycles[chain3].addMonomerToEnd(Atype);
								repeats_added += 1;
							}
							else if (polymers[chain2].repeatUnitAtPosition(i)==Btype)
							{
								cycles[chain3].addMonomerToEnd(Btype);
								repeats_added += 1;
							}
						}
					}
				...
				else // if selected_chain != chain2 D
				{
					// for loops to steal sequence from ester_id to end and add to a 
					// find chain with ester, start there, transfer to selected_chain
					if (ester_found && !is_cyclic) // it's on a linear chain
					{
						for (int i=ester_id;i<=polymers[chain2].getDegreeOfPolymerization()-1;i++)
						{
							if (polymers[chain2].repeatUnitAtPosition(i)==Atype)
					...
					else if (ester_found && is_cyclic) // adds the whole cycle and to selected_chain E
					{
						// start at ester_id on chain2 (cycle) until "end"
						for (int i=ester_id;i<=cycles[chain2].getDegreeOfPolymerization()-1;i++)
						{
	...
	//cout << "Now printing first line of model data package" << endl;
	//cout << package(0, 0) << " " << package(1, 0) << " " << package(2, 0) << endl;
				...
				VectorXd x_model,VectorXd y1_model,VectorXd y2_model)
{
	...
	double sum=0.;
	//cout << "Sum over time = " << sum << " ";
	for (int i=0;i<=x_data.size()-1;i++)
	{
		sum += pow(( y_at_x(x_data(i),x_model,y1_model) - y1_data(i) ),2.);
		//cout << y1_data(i) << " " << y_at_x(x_data(i),x_model,y1_model) << " " << sum << endl;
	}
	for (int ii=0;ii<=x_data.size()-1;ii++)
	{
		sum += pow(( y_at_x(x_data(ii),x_model,y2_model) - y2_data(ii) ),2.);
		//cout << y2_data(ii) << " " << y_at_x(x_data(ii), x_model, y2_model) << " " << sum << endl;;
	}
	return sum;
}
/****************************************************************************************/
	...
	// if the new point is better.
{
	...
	ytry = residual_time(x_data,y1_data,y2_data,time_model,conv_model_A,conv_model_B);
...
//////////////////////////////////////
	...
	for (int i=0;i<mpts;i++)
	{
		...
		conv_model_A =  model_data_package.col(1);
		conv_model_B =  model_data_package.col(2);
		cout << "model data package first and second lines" << endl;
		cout << model_data_package(0, 0) << " " << model_data_package(0,1) << " " << model_data_package(0, 2) << endl;
		cout << model_data_package(1, 0) << " " << model_data_package(1, 1) << " " << model_data_package(1, 2) << endl;
		cout << "calling residual_time at this point" << endl;
		y(i) = residual_time(x_data,y1_data,y2_data,time_model,conv_model_A,conv_model_B);
		//y(i)=func(x);
	...
	for (;;)
	{
		...
		cout << "current y(ilo) = " << y(ilo) << " is less than accuracy goal? " << ftol << endl;
...
//////////////////////////////////////
	...
	for (;;)
	{
		...
		get_psum(p, psum, ihi);
		double rtol=2.0*abs(y(ihi)-y(ilo))/(abs(y(ihi))+abs(y(ilo))+TINY);
		cout << "current rtol = " << y(ilo) << " is less than accuracy goal? " << ftol << endl;
		// Compute the fractional range from highest to lowest and return if satisfactory.
		if (y(ilo) < ftol) //(rtol < ftol)
		{ // If returning, put best point and value in slot 0.
			swap(y[0],y[ilo]);
...
VectorXd	cg_optimization8(VectorXd parameters_old,VectorXd concentrations,int number,
				VectorXd x_data,VectorXd y1_data,VectorXd y2_data,double accuracy_goal)
{
	// derivative of SSR wrt each parameter k_j
	// go down steepest descent direction
	// direction(parameters) = -grad[SSR(parameters)]
	//
	const int	NMAX = 5000;
	int			alt = 0, neu = 1;
	int			iterations = 0;
	...
	int			n_params = parameters_old.size();
	// a vector of VectorXd will store the past directions
	vector< VectorXd >	direction(n_params);
	// a vector of VectorXd will store past gradients to create conjugate directions
	vector< VectorXd >	gradient(n_params);
	// 
	VectorXd	grad = VectorXd::Zero(parameters_old.size());
	// stochastic calculation populates this:
	MatrixXd	model_data_package;
	VectorXd	time_model;
	VectorXd	conv_model_A,conv_model_B;
	VectorXd	parameters_new;
	
	double		t_final = x_data(x_data.size()-1);
	double		convergence;
	double		dk = 1.e-4;
	double		SSR0,SSR;
	double		size = 0.01;		// default linesearch factor
	VectorXd	lambda = VectorXd::Zero(3);
	VectorXd	rk = VectorXd::Zero(3);
	double		brent_size,lin_size;	
	double		r = 10.;			// residual set to something large
	double		rlin,rbrent;
	
	while (r > accuracy_goal)
	{
		size = 0.01; // need to reset this each time
		iterations += 1;
		// Define search direction using derivative information
		// the following 3 can be done in parallel
		// 1
		// generate x_model,y_model using parameters_old
		//kp = parameters_old(0);
		//kd = parameters_old(1);
		model_data_package = stochastic8(concentrations,number,parameters_old,t_final); // still univariate
		time_model =  model_data_package.col(0);
		conv_model_A =  model_data_package.col(1);
		conv_model_B =  model_data_package.col(2);
		SSR0 = residual_time(x_data,y1_data,y2_data,time_model,conv_model_A,conv_model_B);
		
		for (int i=0;i<=parameters_old.size()-1;i++)
		{
			// Calculate a gradient - FOR LOOP HERE
			// 2
			// SSR(kp + dk)
			parameters_old(i) += dk;
			model_data_package = stochastic8(concentrations,number,parameters_old,t_final); // still univariate
			...
			conv_model_A =  model_data_package.col(1);
			conv_model_B =  model_data_package.col(2);
			SSR = residual_time(x_data,y1_data,y2_data,time_model,conv_model_A,conv_model_B);
			grad(i) = (SSR - SSR0)/dk;
			parameters_old(i) -= dk;
			// 3
			// SSR(kd + dk)
			//parameters_old(1) += dk;
			//model_data_package = stochastic2(concentrations,number,parameters_old,t_final); // still univariate
			//time_model =  model_data_package.col(0);
			//conv_model =  model_data_package.col(1);
			//SSR = residual(x_data,y_data,time_model,conv_model);
			//grad(1) = (SSR - SSR0)/dk;
			//parameters_old(1) -= dk;
			//						---- TO HERE
		}
		// two parameters - derivative wrt to each parameter to set initial gradient direction
		// do linesearch in direction -grad
	
		// need to do one gradient step
		
		gradient[neu] = grad;
		
		if (iterations > 1)
		{
		// do conjugate gradient search direction.
			direction[neu] = -gradient[neu] ;
					// + direction[alt]*(gradient[neu].transpose()*gradient[neu])
					//	 /(gradient[alt].transpose()*gradient[alt]);
		}
		else
		{
			direction[neu] = -gradient[neu];
		...
		// line search
		///////////////////////////////////////////
		// the three following can be done in parallel
		lambda(0) = size - dk;		// lower
		parameters_new = parameters_old + lambda(0)*direction[neu];
		// do stochastic integration of time_model and conv_model
		model_data_package = stochastic8(concentrations,number,parameters_new,t_final);
		time_model =  model_data_package.col(0);
		conv_model_A =  model_data_package.col(1);
		conv_model_B =  model_data_package.col(2);
		rk(0) = residual_time(x_data,y1_data,y2_data,time_model,conv_model_A,conv_model_B);
	
		lambda(2) = size + dk;		// upper
		parameters_new = parameters_old + lambda(2)*direction[neu];
		// do stochastic integration of time_model and conv_model
		model_data_package = stochastic8(concentrations,number,parameters_new,t_final);
		time_model =  model_data_package.col(0);
		conv_model_A =  model_data_package.col(1);
		conv_model_B =  model_data_package.col(2);
		rk(2) = residual_time(x_data,y1_data,y2_data,time_model,conv_model_A,conv_model_B);
	
		lambda(1) = size ;		// back to middle
		parameters_new = parameters_old + lambda(1)*direction[neu];
		// do stochastic integration of time_model and conv_model
		model_data_package = stochastic8(concentrations,number,parameters_new,t_final);
		time_model =  model_data_package.col(0);
		conv_model_A =  model_data_package.col(1);
		conv_model_B =  model_data_package.col(2);
		rk(1) = residual_time(x_data,y1_data,y2_data,time_model,conv_model_A,conv_model_B);
	
		cout << "{{" << lambda(0) << ",\t" << rk(0) << "}," << endl;
		cout << "{" << lambda(1) << ",\t" << rk(1) << "}," << endl;
		cout << "{" << lambda(2) << ",\t" << rk(2) << "}}" << endl;

		// Brent's method prediction reset to k  - Polak-Ribiere for multiparameter
		brent_size = brents_optim(lambda(0),lambda(1),lambda(2),rk(0),rk(1),rk(2));
		
	//	cout << "brent_size = " << brent_size << " to min? " << is_it_a_minimum(lambda,rk) << endl;
		// do stochastic integration of time_model and conv_model	
		//parameters_new = parameters_old + brent_size*direction[neu];		
		// if any parameters are negative, keep old parameter
		parameters_new = parameters_old + brent_size*direction[neu];
	//	for (int i=0;i<=parameters_old.size()-1;i++)
	//	{
	//		if (parameters_new(i) < 0.)
	//		{
	//			parameters_new(i) = parameters_old(i); // reset to old value
	//		}
	//	}
		// do stochastic integration of time_model and conv_model	
		//parameters_new = parameters_old + brent_size*direction[neu];
		...
		conv_model_A =  model_data_package.col(1);
		conv_model_B =  model_data_package.col(2);
		// calculate new residual to check convergence
		rbrent = residual_time(x_data,y1_data,y2_data,time_model,conv_model_A,conv_model_B);
		cout << "brent proposed = " << brent_size << " with SSR = " << rbrent <<  " to min? "<< is_it_a_minimum(lambda,rk) << endl;
		rlin = rbrent; // delete later
		if (is_it_a_minimum(lambda,rk)==false) // then get another point and try again
		{
			/* 
			lin_size = linear_optim(lambda(0),lambda(2),rk(0),rk(2));
			
			parameters_new = parameters_old + lin_size*direction[neu];
			for (int i=0;i<=parameters_old.size()-1;i++)
			{
				if (parameters_new(i) < 0.)
				{
					parameters_new(i) = parameters_old(i);
				}
			}
			cout << parameters_new.transpose() << endl;			
			model_data_package = stochastic8(concentrations,number,parameters_new,t_final);
			time_model 	 =  model_data_package.col(0);
			conv_model_A =  model_data_package.col(1);
			conv_model_B =  model_data_package.col(2);
			// calculate new residual to check convergence
			rlin = residual2(x_data,y1_data,y2_data,time_model,conv_model_A,conv_model_B);
			cout << "linear proposed = " << lin_size << " with SSR = " << rlin <<  endl;
 */
			
			while (is_it_a_minimum(lambda,rk)==false)
			{
				if (rk(2) > rk(0))
				{
					lambda(2) = lambda(1); rk(2) = rk(1);
					lambda(1) = lambda(0); rk(1) = rk(0);
					lambda(0) = linear_optim(lambda(1),lambda(2),rk(1),rk(2));
					cout << "λ1 = " << lambda.transpose() << endl;
					parameters_new = parameters_old + lambda(0)*direction[neu];
					for (int i=0;i<=parameters_old.size()-1;i++)
					{
						if (parameters_new(i) < 0.)
				...
				{
					lambda(0) = lambda(1); rk(0) = rk(1);
					lambda(1) = lambda(2); rk(1) = rk(2);
					lambda(2) = linear_optim(lambda(0),lambda(1),rk(0),rk(1));
					cout << "λ2 = " << lambda.transpose() << endl;
					parameters_new = parameters_old + lambda(2)*direction[neu];
					for (int i=0;i<=parameters_old.size()-1;i++)
					{
						if (parameters_new(i) < 0.)
				...
				brent_size = brents_optim(lambda(0),lambda(1),lambda(2),rk(0),rk(1),rk(2));
				cout << "modified brent_size = " << brent_size << " to min? " << is_it_a_minimum(lambda,rk) << endl;
				
				if (is_it_a_minimum(lambda,rk)==true)
				{
					break;
				}
			}
		}
		size = brent_size; r = rlin;
		// take the update that reduced the residual the most between methods
		//cout << "method\tstep\tSSR" << endl;
		if (rbrent <= rlin)
		{
			size = brent_size;
			r = rbrent;
			cout << "brent step taken\t" << brent_size << "\tSSR = " << rbrent << endl;
		}
		else if (rlin < rbrent)
		{
			cout << "sub used\t" << size << "\tSSR = " << rlin << endl;
		}
		//////////////////////// need to keep parameters positive.
		parameters_new = parameters_old + size*direction[neu];
	//	for (int i=0;i<=parameters_old.size()-1;i++)
	//	{
	//		if (parameters_new(i) < 0.)
	//		{
	...
	//		}
	//	}
		convergence = (parameters_new-parameters_old).norm();
		parameters_old = parameters_new;
			cout << "{k} = {" << parameters_new.transpose() << "}, |s_{j+1} - s_{j}| = " << convergence << endl;
		
		// pass gradient and direction from neu to alt
		direction[alt] = direction[neu];
		gradient[alt] = gradient[neu];
		// as we get close, then increase n and decrease dk
		
		// exit if too many iterations
		if (iterations > NMAX)
		{
			cout << "Conjugate gradient failed to converge within max iterations!" << endl;
			return parameters_new;
		}
	}
	return parameters_new; // for a single
}
/****************************************************************************************/
double	linear_optim(double x1,double x2,double fx1,double fx2)
{
